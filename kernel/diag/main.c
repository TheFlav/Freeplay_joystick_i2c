...
int main (int argc, char** argv){
    ...
    bool adc_used[4]={false, false, false, false};
    bool adc_used_prev[4]={false, false, false, false};

    print_stdout("current MCU ADC configuration:\n");
    for (uint8_t i=0; i<4; i++){
        print_stdout("ADC%d: enabled:%d, used:%d\n", i, ((mcu_conf_current.bits >> i+4) & 0b1)?1:0, ((mcu_conf_current.bits >> i) & 0b1)?1:0);
        adc_used[i] = (bool)((mcu_conf_current.bits >> i) & 0b1); adc_used_prev[i] = adc_used[i];
    }
    ...
    for(int draw_x_loop=0, adc_loop=0; draw_x_loop<2; draw_x_loop++){
        for(int draw_y_loop=0; draw_y_loop<2; draw_y_loop++){
            int tmp_line = tty_line + term_adc_vertspacing * draw_y_loop, /*tmp_col = 3 + draw_x_loop * 4, */term_esc_col;
            struct term_adc_pos_t* adc_pos = &term_adc[adc_loop];
            bool enabled = adc_used[adc_loop];
            ...
            //adc "title"
            if (enabled){sprintf (buffer1, "%dbits", adc_data[adc_loop].res); term_esc_col = term_esc_col_normal;} else {sprintf (buffer1, "disabled"); term_esc_col = term_esc_col_disabled;}
            ...
            term_select_add(&term_select[select_index++], 1, &adc_pos->title.x, &adc_pos->title.y, &adc_pos->title.w, 0, 0, false, NULL, adc_pos->title.str, &adc_used[adc_loop], term_hint[2]);
            ...
            //raw min max
            adc_pos->raw_min.x = term_right - 17; adc_pos->raw_max.x = term_right - 6; adc_pos->raw_min.y = adc_pos->raw_max.y = tmp_line; adc_pos->raw_min.w = adc_pos->raw_max.w = 6; //coordinates
            adc_pos->raw_min.value = &adc_data[adc_loop].raw_min; adc_pos->raw_max.value = &adc_data[adc_loop].raw_max; //val pointers
            ...
            if (enabled){
                term_select_add(&term_select[select_index++], 1, &adc_pos->raw_min.x, &adc_pos->raw_min.y, &adc_pos->raw_min.w, 0, 0, true, adc_pos->raw_min.value, NULL, &adc_pos->use_raw_min, term_hint[3]);
                term_select_add(&term_select[select_index++], 1, &adc_pos->raw_max.x, &adc_pos->raw_max.y, &adc_pos->raw_max.w, 0, 0, true, adc_pos->raw_max.value, NULL, &adc_pos->use_raw_max, term_hint[4]);
            }
            tmp_line+=2;
    ...
    //buttons
    struct term_button_pos_t { 
        int x, y, w; char str[buffer_size];
    };
    const int buttons_width = 15;
    const int buttons_count = 3;
    const int buttons_pad = (tty_last_width - buttons_width * buttons_count) / (buttons_count + 1);
    const int buttons_hintoffset = 5;
    char* buttons_name[] = {"Save to boot", "Save to file", "Close"};
    bool* buttons_bool[] = {&kill_resquested, &kill_resquested, &kill_resquested};

    struct term_button_pos_t term_buttons[3] = {0};

    for (int i = 0; i < buttons_count; i++){
        term_buttons[i].x = buttons_pad + (buttons_width + buttons_pad) * i;
        term_buttons[i].y = tty_last_height - 1;
        strcpy(buffer, buttons_name[i]); term_buttons[i].w = array_pad(buffer, strlen(buffer), buttons_width, ' ', 0);
        strcpy(term_buttons[i].str, buffer);
        term_select_add(&term_select[select_index++], 1, &term_buttons[i].x, &term_buttons[i].y, &term_buttons[i].w, 0, 0, false, NULL, term_buttons[i].str, &kill_resquested, term_hint[buttons_hintoffset + i]);
    }
    ...
    while (!kill_resquested){
        ...
        for (int i=0; i<select_index; i++){
            ...
            if (term_select[i].size){
                if (selected){
                    if (term_select[i].type == 1 && select_select && term_select[i].ptrbool){ //set button pressed while selected element is bool
                ...
                if (select_update || term_select[i].force_update){
                    ...
                    if (term_select[i].ptrchar){ //char
                        fprintf(stdout, "\e[%d;%dH\e[%d;%d;%dm%s\e[0m", *term_select[i].y, *term_select[i].x, tmpcol_style, tmpcol_bg, tmpcol_txt, term_select[i].ptrchar);
                    } else if (term_select[i].ptrint){ //int
                        char fmtbuffer[10]; sprintf(fmtbuffer, "%%%dd", *term_select[i].size);
                        char buffer[*term_select[i].size + 1]; sprintf(buffer, fmtbuffer, *term_select[i].ptrint);
                        fprintf(stdout, "\e[%d;%dH\e[%d;%d;%dm%s\e[0m", *term_select[i].y, *term_select[i].x, tmpcol_style, tmpcol_bg, tmpcol_txt, buffer);
                    }
                }
            }
        }
        ...
        for(int i=0; i<4; i++){ //adc loop
            if (adc_used[i] != adc_used_prev[i]){ //adc configuration update
                if ((mcu_conf_current.bits >> i+4) & 0b1){ //enabled
                    mcu_conf_current.bits ^= 1U << i; //toggle needed bit
                    //TODO i2c adc config update
                    adc_used_prev[i] = adc_used[i];
                } else {adc_used[i] = adc_used_prev[i] = false;} //adc not enabled
