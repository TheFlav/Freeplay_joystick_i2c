...
struct freeplay_joy {
    ...
    u32 analogsticks;    //default to digital only 0=digital, 1=single analog stick (2ADCs), 2=dual analog sticks (4ADCs)
    u32 digitalbuttons;  //number of digital inputs to turn on and report to the system
    u32 dpads;           //1=process/report dpad up/down/left/right, 0=no dpad
};
...
static int freeplay_probe(struct i2c_client *client)
{
    ...
    err = device_property_read_u32(&client->dev, "num-analogsticks", &priv->analogsticks);
    if(err)
    {
        priv->analogsticks = 0;
    ...
    err = device_property_read_u32(&client->dev, "num-digitalbuttons", &priv->digitalbuttons);
    if(err)
    {
        priv->digitalbuttons = 11;
        dev_info(&client->dev, "Freeplay i2c Joystick: error reading digitalbuttons property\n");
    }

    err = device_property_read_u32(&client->dev, "num-dpads", &priv->dpads);
    if(err)
    {
        priv->dpads = 1;
    ...
    dev_info(&client->dev, "Freeplay i2c Joystick: analogsticks=%d digitalbuttons=%d dpad=%d\n", priv->analogsticks, priv->digitalbuttons, priv->dpads);

    if(priv->analogsticks == 1)
        adc_mask = 0x03;
    else if(priv->analogsticks == 2)
        adc_mask = 0x0F;
    else
        adc_mask = 0x00;
